...
class Model:
    ...
    def compile(
        ...
        if backend_name == "tensorflow.compat.v1":
            ...
            self._compile_pytorch(lr, loss_fn, decay, loss_weights, clamp)
        elif backend_name == "jax":
    ...
    def _compile_pytorch(self, lr, loss_fn, decay, loss_weights, clamp):
        """pytorch"""
        ...
        def train_step(inputs, targets, auxiliary_vars):
            ...
            if clamp:
                with torch.no_grad():
                    for par in self.external_trainable_variables:
                        par.absolute_()
    ...
    def get_weights(self):
        if backend_name == "tensorflow":
            return [np.array(i) for i in self.net.get_weight_paths().values()]
        elif backend_name == "pytorch":
    ...
    def constrain_params(self, mn=None, mx=None):
        with torch.no_grad():
            for param in self.external_trainable_variables:
                print(param)
                torch.clamp(param, min=mn)
